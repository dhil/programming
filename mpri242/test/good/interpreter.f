...
(* The type constructors [nil] and [cons] are used to encode type-level
   lists. These lists are used to encode typing environments [e], which
   in turn are used as indices in the definition of the type [env e] of
   runtime environments -- see below. *)
